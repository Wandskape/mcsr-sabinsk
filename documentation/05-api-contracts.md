...
Preview и импорт учитывают только участников, активных перед номером
импортируемого матча. UUID ранее выбывших участников попадают в
проигнорированные и не создают результат. Reimport пересчитывает накопительное
выбывание и агрегаты лидерборда.
...
- `GET /admin/divisions/:divisionId/playoff`
...
Сетка создаётся только для участвующего дивизиона в статусе
`QUALIFICATION` или `PLAYOFF`. В дивизионе должно быть не меньше регистраций,
чем выбранный размер сетки. Размер после создания не меняется. Backend создаёт
все позиции основных раундов и сохраняемую позицию матча за третье место;
флаг `showThirdPlace` управляет только её отображением. В `PlayoffSeed`
фиксируются top-N лидерборда по очкам, среднему времени, нику и Ranked UUID.
...
Победителя администратор выбирает вручную, после чего backend автоматически
переносит его в связанную позицию следующего раунда. Проигравшие полуфиналов
переносятся в матч за третье место. Исправление раннего результата сбрасывает
зависимые результаты. Каждое изменение увеличивает версии затронутых матчей и
сетки. Для публикации должны быть заполнены обе позиции всех матчей первого
раунда.
...
Публичная запись семантического просмотра. Принимает `type` (`TOURNAMENT`,
`PARTICIPANT`, `MATCH`) и UUID объекта. Возвращает `{ accepted: boolean }`.
Cookie и постоянный visitor ID не создаются.

### `GET /admin/analytics`

Защищённая административная сводка. Query `period` принимает `TODAY`, `7_DAYS`,
`30_DAYS` или `ALL_TIME`. Возвращает суммы трёх типов просмотров и заполненный
нулевыми днями временной ряд.
